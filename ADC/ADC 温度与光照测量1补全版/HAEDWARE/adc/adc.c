...
void AdcInitForInternalChannels(void) //端口，ADC1初始化内部通道16、17
{
	RCC->CFGR &= ~(3<<14); //分频因子清零
	RCC->CFGR |= 2<<14; //6分频  SYSCLK/DIV2=12M ADC时钟设置为12M,ADC1最大时钟不能超过14M!
	...
	ADC1->SMPR1 |= 7<<18; //239.5周期，提高采用时间可以提高精确度
	ADC1->SMPR1 |= 7<<21; //239.5周期，提高采用时间可以提高精确度
